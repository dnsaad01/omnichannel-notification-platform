...
/**
 * Right sidebar (architecture plan §7, NodeConfigPanelComponent): common
 * controls (name, delete) plus one of the five per-type config components,
 * switched on the selected node's type. All config components write into
 * the same plain `config` object shape the backend's NodeDef.config expects
 * — this wrapper just relays configChange upward untouched.
 */
@Component({
